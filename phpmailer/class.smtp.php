...
class SMTP {
  ...
  /**
   *  SMTP reply line ending (dont change)
   *  @var string
   */
  public $CRLF = "\r\n";
  ...
  public function Connect($host, $port = 0, $tval = 30) {
    ...
    if(substr(PHP_OS, 0, 3) != "WIN") {
     ...
     if ($max != 0 && $tval > $max) { // dont bother if unlimited
      @set_time_limit($tval);
     }
  ...
  public function Authenticate($username, $password, $authtype='LOGIN', $realm='', $workstation='') {
    ...
    switch ($authtype) {
      ...
      case 'NTLM':
        ...
        if(! $ntlm_client->Initialize($temp)){//lets test if every function its available
  ...
  */
  ...
  private function get_lines() {
    $data = "";
    $endtime = 0;
    /* If for some reason the fp is bad, dont inf loop */
